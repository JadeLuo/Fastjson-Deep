...
/**
 * JSON类是使用Fastjson最主要的类.你通常会调用这两个方法: {@link #toJSONString(Object)} 和 {@link #parseObject(String, Class)}.
 *
 * <p>下面是一个简单类使用fastjson的例子:
 *
 * <pre>
 * Model model = new Model();
 * String json = JSON.toJSONString(model); // 将model对象序列化城JSON字符串
 * Model model2 = JSON.parseObject(json, Model.class); // 将JSON字符串反序列化成Model类型的对象
 * </pre>
 * 
* <p>如果你正在序列化/反序列化的对象是一个{@code ParameterizedType}
 * (i.e. 包含至少一个类型参数,也有可能包含一个类型参数的数组),那么你就必须使用
 * {@link #toJSONString(Object)} 或者 {@link #parseObject(String, Type, Feature[])} 方法.  这里是一个
 * 序列化和反序列化 {@code ParameterizedType} 的例子:
 * 
 * <pre>
 * String json = "[{},...]";
